...
Хорошим тоном является применение префиксной формы именования модулей. Разработчики Embarcadero часто применяют такие названия, как `uMain.pas` или `uHelp.pas`. Префикс "u" означает "unit" \("модуль"\). Допускается использование различных шаблонов имен, однако в рамках одного проекта вид наименований не должен различаться.
