...
Хорошим тоном является применение префиксной формы наименований модулей. Разработчиками из Embarcadero часто используются такие названия, как `uMain.pas` или `uHelp.pas`. Префикс "u" означает "Unit" \("Модуль"\). 
